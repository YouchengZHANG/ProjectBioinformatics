...
#  This script when runs use GridSearchCV() to optimize DST parameters on PSSM model
...
def DST_PSSM_Optimizer(filename):
   ...
   #  Set the DST model and parameters 
    clf7 = DecisionTreeClassifier(class_weight='balanced',min_samples_leaf=20)
    clf7_para = {'max_depth':[None,10,100,500],'max_features':['auto','sqrt','log2'],'min_samples_split':[2,6,10,20]}
    kf = KFold(n_splits=3,shuffle=True,random_state=0)
    GS_PSSM_DST = GridSearchCV(estimator=clf7, param_grid=clf7_para,cv=kf)
    classifier = [GS_PSSM_DST]
    ...
    for ws_left,ws_right in [[9,9]]:
        ...
        #  Train the GridSearchCV(DST) model and save the output to .cv_results_ a dictionary file
        ...
        for modelnum,model in enumerate(classifier):
            ...
            joblib.dump(model.cv_results_,'GS_DST_PSSM.dict')
