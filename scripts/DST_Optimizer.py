...
#  This script when runs use GridSearchCV() to optimize DST parameters on single sequence model
...
def DST_Optimizer(filename):
    ...
    GS_DST = GridSearchCV(estimator=clf7, param_grid=clf7_para,cv=kf)
    classifier = [GS_DST]
    ...
    for ws_left,ws_right in [[9,9]]:    
        ...
        for modelnum,model in enumerate(classifier): 
            ...
            joblib.dump(model.cv_results_,'GS_DST.dict')
